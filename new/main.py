...
class Game:
    def __init__(self):
        ...
        self.mapList = []
        for i in MAPS:
            self.mapList.append(TiledMap(path.join(map_folder, i))) # add every tiledMap to the list.
        try:
            self.map = self.mapList[CURRENTMAP] # load the Current level's map
        except IndexError: # if the current level's map is outside of the maplist range the just loadthe first map.
            self.map = self.mapList[0]
            self.load_data()
            self.intro_screen(self.endgame_img) # show the game completed message
        self.load_data()
        
    def load_data(self):
        img_folder = path.join(self.game_folder, 'img') #path for main image folder
    ...
    def draw(self):
        pg.display.set_caption("{:.2f}".format(self.clock.get_fps())) # displays the fps
